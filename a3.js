...
// a) i) animation plays
// a) ii) initFileObjects - load objects from path; onResourcesLoad - shallow copy and initiate objects in scene
// a) iii) the keyframe values correspond to x,y,z,theta of the instance
...
function initMotions() {
    ...
    myboxMotion.addKeyFrame(new Keyframe('pose C',2.0, [-3, 7, 0, 0]));
    myboxMotion.addKeyFrame(new Keyframe('pose D',3.0, [0, 7, 0, 0]));
    ...
    handMotion.addKeyFrame(new Keyframe('right finger curl',1.0, [0, 3,    30, +140, -140, 0,0]));
    handMotion.addKeyFrame(new Keyframe('straight',         2.0, [0, 3,    0, 0, 0, 0, 0]));
    handMotion.addKeyFrame(new Keyframe('left finger curl', 3.0, [0, 3,    -30, 0, 0, -90,-90]));
    handMotion.addKeyFrame(new Keyframe('straight',         4.0, [0, 3,    0, 0, 0, 0, 0]));
    handMotion.addKeyFrame(new Keyframe('both fingers curl',4.5, [0, 3,    -30, -90, -90, -90,-90]));
...
function handSetMatrices(avars) {
    ...
    link1.matrix.multiply(M.makeScale(7,1,4));    
...
function onResourcesLoaded(){
 ...
 //                             i.e., creates references to the geometry, and not full copies ]
    ...
    meshes["teapot2"].position.set(0, 0, 3);
